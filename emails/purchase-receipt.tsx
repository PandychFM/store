...
export default async function PurchaseReceiptEmail({
  ...
  return (
    <Html>
      <Preview>Просмотр квитанции о получении заказа</Preview>
      <Tailwind>
        <Head />
        <Body className='font-sans bg-white'>
          <Container className='max-w-xl'>
            <Heading>Квитанция о покупке</Heading>
            <Section>
              <Row>
                <Column>
            ...
            <Section className='border border-solid border-gray-500 rounded-lg p-4 md:p-6 my-4'>
              ...
              {[
                { name: 'Товар', price: order.itemsPrice },
                { name: 'Налог', price: order.taxPrice },
                { name: 'Доставка', price: order.shippingPrice },
                { name: 'Сумма', price: order.totalPrice },
              ].map(({ name, price }) => (
